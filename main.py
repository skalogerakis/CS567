...
# UPDATED: Comment out rule 19. TODO change that in the report as well
...
# UPDATED: Comment out rule 5. TODO change that in the report as well
...
def is_consistent(ontology, flag):
    # print(list(ontology.inconsistent_classes()))
    # Despite we care mainly for the error generated by the reasoner, also check whether the ontology
    # has any inconsistent classes. In most of the cases, this list is empty but add this case to make the check bulletproof
...
def is_of_types(individual):
    ...
    concepts = set()    # Initially use a set to avoid duplicate values in the final result and then cast it to list in return
    for parents in onto.get_parents_of(individual):
...
def get_all_concept_instances(concept):
    # onto.get_instances_of(concept) returns just the defined instances. We also want the inferred instances, so use that one
...
def is_subclass_of(C, D):
    subclasses = onto.get_children_of(D)    # This method does not only support Concepts that we care about, but also roles
    if C in subclasses:
        return True
    return False
...
if __name__ == '__main__':
    ...
    with onto:      # Use in this manner, otherwise the reasoner's changes do not apply in the next questions
        ...
        # If we want to save the modified .owl file by the reasoner
